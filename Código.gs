...
var CONFIG = {
  ...
  TRAMOS_PROP: 'TRAMOS_COMISION_JSON',   // tramos de comision (coordinadores y ejecutivos)
...
function loadVentas_(){
  ...
  for(var i=1;i<v.length;i++){
    ...
    slim.push({t:d.getTime(), a:ag, u:us, m:am, p:pr, f:fn});
  }
  _cachePut_('VEN', slim, 300);
  return rows;
}
...
function loadAusentismo_(fromDate, toDate){
  var res={byNk:{}, rutByNk:{}, disponible:false, mensaje:''};
  try{
    ...
    var iId=cc['id'], iRol=cc['rol_operativo'], iRut=cc['rut'];
    var rolById={}, rutById={};
    ...
    Object.keys(nkById).forEach(function(cid){ if(rolById[cid]==='EJECUTIVO') res.rutByNk[nkById[cid]]=rutById[cid]||''; });
    ...
    var LABEL ={RETRASO:'Retraso', SALIDA_ANTICIPADA:'Salida anticipada',
      ...
      MIXTO:'Retraso + salida anticipada', SIN_MARCAJE_SALIDA:'No marca salida', LICENCIA_MEDICA:'Licencia medica'};
    ...
    Object.keys(allKeys).forEach(function(key){
      ...
      if(mm){   // ---- registro MANUAL (prevalece) ----
        ...
        if(DEDUCT[tc2]||OTRAS_BSC[tc2]){
          ...
          var tsel=(tc2==='RETRASO')?'RETRASO':(tc2==='SALIDA_ANTICIPADA')?'SALIDA_ANTICIPADA':(tc2==='INASISTENCIA_INJUSTIFICADA')?'INASISTENCIA':(tc2==='MIXTO')?'MIXTO':'SIN_INCIDENCIA';
...
function guardarIncidenciaManual(nk, agente, fecha, tipo, justificada, descuenta, minutos, comentario){
  ...
  var TIPOS={RETRASO:1,SALIDA_ANTICIPADA:1,INASISTENCIA:1,MIXTO:1,SIN_INCIDENCIA:1};
...
function getMetas(){
  ...
  if(!raw) return {grupo:0, metaAgente:0, metaCoordinador:0};
  try{ var o=JSON.parse(raw); return {grupo:Number(o.grupo)||0, metaAgente:Number(o.metaAgente)||0, metaCoordinador:Number(o.metaCoordinador)||0}; }
  catch(e){ return {grupo:0, metaAgente:0, metaCoordinador:0}; }
}
/** grupo: meta mensual total; metaAgente: meta por agente; metaCoordinador: meta por coordinador (misma para todos). */
function saveMetas(grupo, metaAgente, metaCoordinador){
  _bloquearSiRestringido_();
  var obj={grupo:Number(grupo)||0, metaAgente:Number(metaAgente)||0, metaCoordinador:Number(metaCoordinador)||0};
...
/** Devuelve {coord:[...5], ejec:[...5]} con {inf, sup(null=en adelante), pct}. */
function getTramos(){
  var def={
  ...
  var raw=PropertiesService.getScriptProperties().getProperty(CONFIG.TRAMOS_PROP);
  if(!raw) return def;
  try{ var o=JSON.parse(raw); return {coord:_normTramos_(o.coord)||def.coord, ejec:_normTramos_(o.ejec)||def.ejec}; }
  catch(e){ return def; }
}
function saveTramos(coord, ejec){
  _bloquearSiRestringido_();
  var obj={coord:_normTramos_(coord)||[], ejec:_normTramos_(ejec)||[]};
  PropertiesService.getScriptProperties().setProperty(CONFIG.TRAMOS_PROP, JSON.stringify(obj));
  return obj;
}
...
function getDashboard(params){
  ...
  ventas.forEach(function(s){
    ...
    if(esEjec){
      ...
      if(withDetail) (ventasDet[ven.nk]=ventasDet[ven.nk]||[]).push({fecha:s.dateKey, producto:s.product, monto:round_(s.amount), user:s.user});
    } else {
  ...
  var metas=getMetas();
  var tramos=getTramos();
  var t5e=tramos.ejec[tramos.ejec.length-1]||{inf:0}, t5c=tramos.coord[tramos.coord.length-1]||{inf:0};
  var metaAg=t5e.inf||0, metaCo=t5c.inf||0;   // meta individual = inicio del Tramo 5
  ...
  return {
    ...
    tramos:tramos,
    resumen:resumen,
    metas:{grupo:metas.grupo||0, metaAgente:metaAg, metaCoordinador:metaCo},
...
function getMesAgentes(periodo){
  ...
  return {rango:d.rango, periodo:R.periodo, mesLabel:R.mesLabel, corte:d.corte,
          ejecutivos:ag, acceso:{esAgente:acc.esAgente, esCoordinador:acc.esCoordinador, esSupervisor:acc.esSupervisor, nombre:acc.nombre, identificado:acc.identificado}};
}
...
function getMesCoordinadores(periodo){
  ...
  return {rango:d.rango, periodo:R.periodo, mesLabel:R.mesLabel, corte:d.corte,
          coordinadores:lista, acceso:{esAgente:acc.esAgente, esCoordinador:acc.esCoordinador, nombre:acc.nombre, identificado:acc.identificado}};
}
...
function exportCSV(tipo,params){
  ...
  if(tipo==='ejecutivos'){ rows.push(['Ejecutivo','RUT','Supervisor','Ventas','Viajes','Compartida','Exclusiva','Dia','Noche','Nota_Auditoria','%_Registros_OK','%_Adherencia','Meta','%_Avance','Comision','Comision_Real']);
    d.ejecutivos.forEach(function(e){rows.push([e.nombre,(e.rut||''),e.sup,e.ventas,e.n,e.comp,e.excl,e.dia,e.noche,e.notaAuditoria,e.pctOk,(e.adherencia==null?'':e.adherencia),e.meta,e.avance,e.comision,e.comisionReal]);}); }
...
/** Resumen de desempeño por agente: Ventas | Nota Auditoría | % Registros Correctos. */
function generarResumenDesempenoPDF(params){
  ...
  var data=[['Agente','Ventas','Nota Auditoría','% Registros Correctos']];
...
function generarDetalleDesempenoPDF(params, nombreAgente){
  ...
  lista.forEach(function(e, idx){
    ...
    _tabla_(b, [['Ventas','Viajes','Nota Auditoría','% Registros Correctos'],
      [_fmtCLP_(e.ventas), String(e.n),
       e.notaAuditoria==null?'—':String(e.notaAuditoria),
       e.pctOk==null?'—':(Math.round(e.pctOk*100)+'%')]]);
    _sub_(b,'Detalle de ventas del periodo ('+((e.ventasDetalle||[]).length)+')');
    var vt=[['Fecha','Servicio','Monto','Correo pasajero']];
    (e.ventasDetalle||[]).forEach(function(v){ vt.push([v.fecha, v.producto, _fmtCLP_(v.monto), v.user||'—']); });
