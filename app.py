...
<style>
    ...
    .info-box {background-color: #FFF8E1; padding: 15px; border-radius: 5px; border-left: 5px solid #FFC107; font-size: 14px;}
</style>
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**Reglas de Exclusión (Loza/Colación):**")
    st.caption("• Turno 10:00 -> OFF: 10-11 y 14-16")
    st.caption("• Turno 05:00 -> OFF: 11-14")
    st.caption("• Turno 21:00 -> OFF: 06-09")
...
if st.button("Generar Informe"):
    if t_file and v_file:
        try:
            ...
            if res[0] is None:
                st.error("Error: No se encontró columna de fecha válida en Ventas.")
            else:
                df_h, df_d, df_t, df_s = res
                
                st.markdown("""
                <div class='info-box'>
                <b>Nota:</b> Los coordinadores marcados con <b>(*)</b> en la Matriz Horaria se encontraban en horario de 
                Gestión de Loza/Colación. Durante esas horas, no reciben asignación de ventas (se reparten entre los demás activos).
                </div>
                <br>
                """, unsafe_allow_html=True)
                ...
                with tab3:
                    st.write("##### Análisis de Competencia (Solo horas hábiles de venta)")
