/**************************************************************************************************
 * [CL] Airport Performance · Coordinators & Fleets — v4
...
/* Huella de la data: si el Sapineitor recarga una hoja (cambia su tamaño), la clave
   del caché cambia y todo se recalcula. Evita servir cifras de una carga anterior. */
function _fingerprint() {
  var names = [SH.LOSA, SH.JOURNEY, SH.PICKUP, SH.RESCATE, SH.PROG, SH.CONEX];
  return names.map(function (n) {
    try { var sh = _sheet(n); return sh.getLastRow() + 'x' + sh.getLastColumn(); }
    catch (e) { return '0'; }
  }).join('|');
}
...
/* Deja el resumen caliente. Ideal: trigger unos minutos después del Sapineitor. */
function precalentarCache() {
  var b = _monthBounds();
  _shift(true);
  getSummary({ dateFrom: b.defFrom, dateTo: b.max, hours: [], coordinator: '__ALL__', nocache: true });
...
function getSummary(filters) {
  var cache = CacheService.getScriptCache();
  var key = _ckey('sum5_', [filters.dateFrom, filters.dateTo, filters.hours || [],
    filters.coordinator || '', _fingerprint()]);

  if (!filters.nocache) {
    var hit = cache.get(key);
    if (hit) return JSON.parse(hit);
  }
  var out = _computeSummary(filters, !!filters.nocache);
  try { cache.put(key, JSON.stringify(out), CACHE_TTL_SEC); } catch (e) { /* >100KB: sin caché */ }
  return out;
}

function _computeSummary(filters, force) {
...
function getCellDetail(dk, hr, coordinator) {
  hr = Number(hr);
  var cache = CacheService.getScriptCache();
  var key = _ckey('det2_', [dk, hr, coordinator || '', _fingerprint()]);
  var hit = cache.get(key);
  if (hit) return JSON.parse(hit);
  ...
  try { cache.put(key, JSON.stringify(result), CACHE_TTL_SEC); } catch (e) { /* muy grande */ }
  return result;
}
