...
def process_all(sales_file, turnos_file, start_date, end_date):
    ...
    for name in coord_list:
        ...
        if name in turnos:
            for d in turnos[name]:
                if start_date <= d <= end_date:
                    worked_days += 1
        total_metrics.append({'Coordinador': name, 'Ventas Totales': round(v), 'Turnos Trabajados': worked_days})
        
        solo, con1, con2plus = 0, 0, 0
        ...
        shared_stats.append({'Coordinador': name, 'Solo (Franjas)': solo, 'Con 1 más (Franjas)': con1, 'Con 2 o más (Franjas)': con2plus})
